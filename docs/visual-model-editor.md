...
- 已应用的画布位置同步到本机实验目录，界面存储仅作缓存，按实验与节点顺序区分，保留最近 20 组布局；位置不进入物理模型、不参与 KPI 计算。没有可用的布局记录时，自动按模型生成画布。
...
图形草稿不会替代既有模型或运行结果。当前 main 已实现名单第 5 项：草稿、撤销记录和布局同步到本机实验目录，重开后可恢复；冲突仍需用户核对。详见 [持久草稿与备份](productivity-update.md)。
