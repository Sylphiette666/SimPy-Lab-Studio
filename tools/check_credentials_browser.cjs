...
(async () => {
  ...
  page.on('request', request => { if (request.url().endsWith('/adjust')) aiCalls++; });
  ...
  try {
    ...
    checks.push('Explicit opt-in, saved status, blank password field and cancel without mutation.');
