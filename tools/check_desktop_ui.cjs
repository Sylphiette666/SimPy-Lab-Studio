...
(async () => {
  ...
  try {
    ...
    assert.ok(page, 'Native WebView2 window must load the local app');
    const errors = [];
    ...
    const workspace = await page.locator('#model-dialog').count() > 0;
    if (workspace) await page.locator('#edit-model').click();
    ...
    if (workspace) await page.locator('#nav-results').click();
    ...
    if (workspace) {
      await page.screenshot({path:path.join(imageRoot, 'workspace-results.png')});
    ...
    await page.screenshot({path:path.join(imageRoot, workspace ? 'workspace-preview.png' : 'desktop-studio.png')});
    await page.locator('#ai-settings').click();
    await page.screenshot({path:path.join(imageRoot, workspace ? 'workspace-models.png' : 'desktop-models.png')});
