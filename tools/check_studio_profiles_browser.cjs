...
(async () => {
  ...
  try {
    ...
    const adjusted = page.waitForResponse(r => r.url().endsWith('/apply') && r.request().method() === 'POST');
    ...
    await page.locator('#ai-settings').click();
    await page.waitForFunction(() => document.querySelector('#ai-api-key').value === 'fixture-only-key-1');
