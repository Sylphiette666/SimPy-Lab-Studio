...
[下载测试版源码 ZIP](source-packages/SimPy-Lab-Studio-test-source.zip?raw=true) · [源码包说明与校验](source-packages/README.md)

> 当前分支 `frontend/simulation-workspace` 为持续更新的测试版，合并保留界面改版、设备与容器编辑及 [AI 响应完整性修复](docs/ai-response-fix.md)。今后在此分支更新并覆盖测试版交付文件，不再另建多份软件。第一版 `main` 和 v1.0.0 发布保持原样。SimPy 引擎和指标计算保持不变；设计与交互说明见 [工作台改版说明](docs/workspace-redesign.md)。
...
**以下下载提供第一版 v1.0.0，并不包含本分支的测试版功能。** 第一版主分支、EXE、安装程序和发布记录继续保留；测试版持续更新于本分支，可按下文从源码启动。
...
1. 打开本分支应用，点击“编辑模型”或左侧“模型”，修改初始输入；所有机器、缓冲区、班次和实验设置都在输入模型窗口中。点击窗口外的遮罩即可返回主界面，再次打开时保留当前草稿；仍需点击“应用修改并预览”才会保存并运行。
...
本机软件固定保留 `SimPy Lab Studio/SimPy Lab Studio.exe`（第一版）和 `SimPy Lab Studio 测试版/SimPy Lab Studio.exe`（最新测试版）。以后通过验证的新版本覆盖测试版，旧的界面预览版、设备扩展版和 AI 修复版不再单独分发。源码包分别保留第一版与测试版，Git 历史仍可追溯各次修改。
