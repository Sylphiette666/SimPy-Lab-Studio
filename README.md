...
> 当前分支 `frontend/simulation-workspace` 为仿真工作台界面预览版。中央仿真画面、全屏预览、旁侧指标与运行助手、结束后的方案评估均使用原有仿真接口；SimPy 引擎和指标计算保持不变。另包含 [AI 响应完整性修复](docs/ai-response-fix.md)。设计来源、交互边界与验证方法见 [工作台改版说明](docs/workspace-redesign.md)。
